...
<xsl:variable name="settingsfilename" select="concat($Xbaserealdir, '/../data/config/settings.xml')"/>
<xsl:variable name="settingsdocument" select="document($settingsfilename)"/>
<xsl:variable name="backgroundColor" select="document($settingsfilename)"/>
<xsl:variable name="footer" select="document($settingsfilename)"/>
   <xsl:variable name="logoValue" select="$settingsdocument/settings/logo"/>

   <xsl:param name="titleParam"/>
   <xsl:param name="bgColorParam"/>
   <xsl:param name="logo"/>
<xsl:output method="html" omit-xml-declaration="yes" encoding="UTF8"/>

<xsl:template name="htmlheader">
   <head>
      <!--  <title>NDLTD Document8</title>  -->
      <title><xsl:value-of select="$settingsdocument/settings/title"/></title>
      <!-- Google sign-in -->
...
<!--      <meta http-equiv="Cache-Control" content="no-store"/>
      ...
      <style>
         body{
            background-color: <xsl:value-of select="$backgroundColor/settings/backgroundColor"/> !important;
         }
      </style>
   </head>
</xsl:template>
...
<xsl:template name="banner">
   <div class="banner">
      <div class="banner-logo"><a href="{concat ($basedir, 'index.html')}">
         <img src="{concat ($basedir, 'images/', $logoValue)}"/></a></div>
            ...
            <xsl:for-each select="$collectionsdocument/item">
               ...
               <div class="collectionbox">
                  ...
                  <div class="collectionimage">
                     <img src="{concat($basedir, 'thumbs/', $collectiondocument/view/fileDetails, '.jpg')}" alt="{$collectiondocument/title}"/>
                  </div>
         ...
         <div class="browsesection linesection">
            ...
            <div class="collectionbox">
               <a href="metadata/FHYA Depot/index.html">
                  <div class="collectionimage">
                     <img src="{concat($basedir, 'thumbs/', $collectiondocument/view/fileDetails, '.jpg')}" alt="FHYA Depot"/>
                  </div>
                  <div class="collectiontitle">
                     FHYA Depot
                  </div>
            ...
            <div class="collectionbox">
               <a href="metadata/Public Depot/index.html">
                  <div class="collectionimage">
                     <img src="{concat($basedir, 'thumbs/', $collectiondocument2/view/fileDetails, '.jpg')}" alt="Public Depot"/>
                  </div>
                  <div class="collectiontitle">
                     Public Depot
                  </div>
...
-->
         ...
         <div class="aboutfhya linesection">
            ...
            <p><xsl:value-of select="$settingsdocument/settings/footer"/></p>
...
<!--            <xsl:for-each select="comment|upload|contribution">
               <xsl:choose>
                  <xsl:when test="local-name()='comment'">
                     <xsl:variable name="commentsfilename" select="concat ($Xbaserealdir, '/comments/', fileDetails)"/>
                     <xsl:variable name="commentsdocument" select="document(exsl:encode-uri ($commentsfilename, false ()))"/>
                     <div class="metadataField">
